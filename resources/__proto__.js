module.exports = Object.assign({}, require('../lib/MyObject'), {
  ...
  badRequest() {
    ...
    return new Promise(resolve => {
      ...
      this.response.writeHead(
        data.code || 200,
        Object.assign(this.getHeaders(data.body), data.headers || {})
      );
  ...
  getQs() {
    this.query = this.qs.length
      ? JSON.parse(this.QueryString.unescape(this.qs))
      : {};
  },
  ...
  slurpBody() {
    return new Promise((resolve, reject) => {
      ...
      this.request.on('end', () => {
        try {
          resolve((this.body = JSON.parse(body)));
        } catch (e) {
  ...
  validateUser() {
    const permissions =
      this.Permissions[this.path[0]] &&
      this.Permissions[this.path[0]][this.request.method];

    if (
      (this.user.id === undefined && this.request.method !== 'GET') ||
      (permissions &&
        this.user.roles.find(role => permissions.has(role)) === undefined)
    ) {
