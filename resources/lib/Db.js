module.exports = Object.create(
  {
    ...
    apply(resource) {
      if (this.Mongo.model[resource.path[0]])
        return this.Mongo[resource.request.method](resource);
    ...
    DELETE() {
      return this.Postgres.query(
        `DELETE FROM "${resource.path[0]}" WHERE id = ${
          resource.path[1]
        } RETURNING id`
      );
    },
    ...
    GET(resource) {
      ...
      if (this.Postgres[`${name}s`])
        return Promise.resolve(this.Postgres[`${name}s`].data);
      ...
      queryKeys.forEach(key => {
        ...
        if (/join/i.test(operation)) {
          let fkCol = this.Postgres.tables[datum.value.table].columns.find(
            column => column.name === datum.value.column
          );
      ...
      selects = Object.keys(selects)
        .map(tableName => this._getColumns(tableName, { extend: joins.length }))
        .join(', ');
    ...
    PATCH(resource) {
      var paramCtr = 1,
        ...
        set =
          'SET ' + bodyKeys.map(key => `"${key}" = $${paramCtr++}`).join(', ');
    ...
    _getColumns(name, opts = {}) {
      return this.Postgres.tables[name].columns
        .map(column => {
          let rv = opts.columnOnly
            ? `"${column.name}"`
            : `"${name}"."${column.name}"`;
