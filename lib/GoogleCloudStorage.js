module.exports = Object.create(
  Object.assign({}, require('./MyObject'), {
    ...
    getToken() {
      if (this.currentToken.expiry_date > new Date().getTime())
        return Promise.resolve(this.currentToken);
      if (this.getTokenPromise) {
        return this.getTokenPromise;
      }

      const client = new this.Google.auth.JWT(
      ...
      this.getTokenPromise = this.P(client.authorize, [], client).then(
        ([token]) => {
    ...
    GET(name) {
      return this.getToken().then(token =>
        this.Request({
          ...
          path: `/storage/v1/b/${
            process.env.GOOGLE_BUCKET
          }/o/${this.QueryString.escape(name)}`,
          protocol: 'https',
          port: 443,
          streamIn: true,
        })
    ...
    POST(name, stream) {
      return this.getToken()
        ...
        .then(([body, response]) =>
          Promise.resolve([response.statusCode, body])
        );
    },
  }),
  {}
