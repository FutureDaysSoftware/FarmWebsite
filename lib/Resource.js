  ...
  Object.assign({}, require('../../lib/MyObject'), {
    ...
    assert(opts) {
      this.Postgres.tables[opts.name].columns
        ...
        .forEach(column =>
          this.Chai.assert.isDefined(
            opts.model[column.name],
    ...
    create(opts = {}) {
      return (opts.token
        ...
        Promise.all(
          Array.from(Array(opts.count).keys()).map(i => {
            const data =
              typeof opts.bootstrap === 'function' ? opts.bootstrap(i) : {};
            return (typeof data.then === 'function'
              ...
              return resource
                .create()
                .then(() => resource.save({ method: opts.method }));
            });
          })
        )
      );
