  ...
  Object.assign({}, require('../lib/MyObject'), {
      ...
      return this.getDb()
        .then(db =>
          db
            ...
            .remove({ _id: this.ObjectId(id || resource.path[1]) })
        )
        .then(result => Promise.resolve([{}]));
    },

    GET(resource) {
      if (resource.path.length === 2)
      ...
      return this.forEach(
        db =>
          db
            .collection(resource.path[0])
            .find(resource.query)
            .skip(cursorMethods.skip)
            .limit(cursorMethods.limit)
            .sort(cursorMethods.sort),
    ...
    POST(resource) {
      return this.getDb()
        ...
        .then(result =>
          Promise.resolve([
            Object.assign({ _id: result.insertedIds[0] }, resource.body),
          ])
        );
    },
    ...
    PUT(resource, id) {
      return this.getDb()
        ...
        .then(result =>
          Promise.resolve([
            Object.assign({ _id: id || resource.path[1] }, resource.body),
          ])
        );
    },
    ...
    handleSingleDocument(resource) {
      return this.getDb().then(db =>
        db
          .collection(resource.path[0])
          .findOne({ _id: this.ObjectId(resource.path[1]) })
      );
    },
    ...
    getDb() {
      return this.Client.connect(process.env.MONGODB);
    },

    removeCollection(collectionName) {
      this.objectNames = this.objectNames.filter(
    ...
    transform(collectionName, document) {
      ...
      this.model[collectionName].attributes.forEach(attribute => {
        if (attribute.fk && document[attribute.fk])
          document[attribute.fk] = new this.Mongo.ObjectID(
            document[attribute.fk]
          );
      });
