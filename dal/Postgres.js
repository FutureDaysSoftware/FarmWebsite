  ...
  Object.assign({}, MyObject, {
    ...
    validateKeys(table, columns) {
      columns.forEach(column => {
        if (!this.tables[table].model.store.name[column])
    ...
    getColumnDescription(table, column) {
      const isEnum =
          this.enumReference &&
          this.enumReference[table.table_name] &&
      ...
      return {
        ...
        maximumCharacterLength:
          column.data_type === 'text' ? 1000 : column.character_maximum_length,
    ...
    getSelectList(table, opts = {}) {
      return typeof table === 'string'
        ...
        : table
            .map(t => this._getSelect(t, opts.alias ? opts.alias[t] : t))
            .join(', ');
    },
    ...
    _getSelect(table, alias) {
      return this.tables[table].columns
        .map(
          column => `"${alias}"."${column.name}" as "${alias}.${column.name}"`
        )
        .join(', ');
    },
    ...
    getTableData() {
      return this.query(this._queries.selectAllTables())
        .then(result =>
          Promise.all(
            result.map(row =>
              this.query(this._queries.selectTableColumns(row.table_name)).then(
                columnResult =>
                  Promise.resolve(
                    (this.tables[row.table_name] = {
                      columns: columnResult.map(columnRow =>
                        this.getColumnDescription(row, columnRow)
                      ),
                    })
                  )
              )
        ...
        .then(() =>
          this.query(this._queries.selectForeignKeys()).then(result =>
            Promise.resolve(
              result.forEach(row => {
                const match = /FOREIGN KEY \("?(\w+)"?\) REFERENCES (\w+)\((\w+)\)/.exec(
                  row.pg_get_constraintdef
                );
                let column = this.tables[
                  row.tablefrom.replace(/"/g, '')
                ].columns.find(column => column.name === match[1]);
        ...
        .then(() => {
          this.objectNames = Object.keys(this.tables);
        ...
        .then(() =>
          this.query('SELECT * FROM division').then(divisions =>
            Promise.resolve(
              (this.divisions = Object.create(this.Model).constructor(
                divisions,
                {
                  storeBy: ['id', 'name'],
                }
      ...
      return Object.create(
        {
          ...
          rollback(e) {
            return this.P(this.client.query, ['ROLLBACK'], this.client)
              ...
              .catch(error => {
                console.log(`Error rolling back: ${error}, ${e}`);
                return Promise.reject(e);
              });
          },
          ...
          transaction(queries) {
            return this.connect().then(() => {
              ...
              queries.forEach(
                query =>
                  (chain = chain.then(() =>
                    this.P(this.client.query, query, this.client).catch(e =>
                      this.rollback(e)
                    )
                  ))
              );
              ...
              return chain.then(() =>
                this.P(this.client.query, ['COMMIT'], this.client).then(() =>
