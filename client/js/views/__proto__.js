...
module.exports = Object.assign(
    ...
    require('events').EventEmitter.prototype,
    {
        ...
        constructor(opts = {}) {
            ...
            if (this.requiresLogin && !this.user.isLoggedIn())
        ...
        getEventMethodName(key, event) {
            return `on${this.capitalizeFirstLetter(
                key
            )}${this.capitalizeFirstLetter(event)}`
        },
        ...
        hide(isSlow) {
            ...
            this.isHiding = true
            return this.hideEl(this.els.container, isSlow).then(() =>
                Promise.resolve((this.hiding = false))
            )
        },
        ...
        isAllowed(user) {
            ...
            if (Array.isArray(this.requiresRole)) {
                const result = this.requiresRole.find(role =>
                    userRoles.has(role)
                )
        ...
        render() {
            if (this.data)
                this.model = Object.create(this.Model, {}).constructor(
                    this.data
                )
            ...
            if (this.size) {
                this.size()
                this.OptimizedResize.add(this.size.bind(this))
            }
        ...
        renderSubviews() {
            this.subviewElements.forEach(obj => {
                ...
                if (this.events.views) {
                    if (this.events.views[name])
                        this.events.views[name].forEach(arr =>
                            this.views[name].on(arr[0], eventData =>
                                Reflect.apply(arr[1], this, [eventData])
                            )
                        )
                    else if (this.events.views[obj.view])
                        this.events.views[obj.view].forEach(arr =>
                            this.views[name].on(arr[0], eventData =>
                                Reflect.apply(arr[1], this, [eventData])
                            )
                        )
        ...
        slurpTemplate(options) {
            ...
            Array.from(
                fragment.querySelectorAll(
                    `${selector}, ${viewSelector}, ${imgSelector}`
                )
            ).forEach(el => {
                if (el.hasAttribute(this.slurp.attr)) {
                    this.slurpEl(el)
