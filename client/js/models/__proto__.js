    ...
    require("events").EventEmitter.prototype,
    {
            ...
            return this.Xhr({
                ...
                if (Array.isArray(this.data)) {
                    const datum = this.data.find(
                        datum => datum[key] == keyValue
                    )

                    if (this.store) {
                        Object.keys(this.store).forEach(attr => {
                            this.store[attr][datum[attr]] = this.store[attr][
                                datum[attr]
                            ].filter(datum => datum[key] != keyValue)
                            if (this.store[attr][datum[attr]].length === 0) {
                                this.store[attr][datum[attr]] = undefined
                            }
                        })
                    }
            ...
            return this.Xhr({
                ...
                if (Array.isArray(this.data)) {
                    this.data = this.data.concat(
                        opts.parse
                            ? opts.parse(response, opts.storeBy)
                            : response
                    )
                } else {
        ...
        remove(item) {
            const index = this.data.findIndex(
                datum => JSON.stringify(datum) === JSON.stringify(item)
            )
