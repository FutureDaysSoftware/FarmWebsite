    ...
    Object.assign({}, require('../../lib/MyObject'), {
        ...
        initialize() {
            ...
            this.Singletons.forEach(name =>
                this.Views[name].constructor({ factory: this.ViewFactory })
            )
        ...
        handler(path) {
            ...
            Promise.all(
                Object.keys(this.views).map(view => this.views[view].hide())
            )
                .then(() => {
                    this.currentView = view

                    if (this.views[view])
                        return this.views[view].onNavigation(path)
        ...
        onLogout() {
            Promise.all(
                Object.keys(this.views).map(view => this.views[view].delete())
            )
                .then(() => {
