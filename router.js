  ...
  Object.assign({}, require('./lib/MyObject'), {
    ...
    async initialize() {
      ...
      const fileHash = files
        .filter(name => !/^[\._]/.test(name) && /\.js/.test(name))
        .reduce(
          (memo, name) =>
            Object.assign(memo, {
      ...
      dals.forEach(dal =>
        this[dal].objectNames.forEach(
          name => (this.jsonRoutes[name] = fileHash[name] ? name : '__proto__')
        )
      );
      ...
      Object.keys(fileHash)
        .filter(name => {
          let rv = true;
          dals.forEach(dal => {
            if (this[dal].objectNames[name]) rv = false;
          });
          return rv;
        })
